...
import { disposeSessionQuietly, runAgent } from "../src/agent-runner.js";
...
describe("AgentManager — Bug 3 clearCompleted", () => {
  ...
  it("clearCompleted calls disposeSessionQuietly on sessions of removed records", async () => {
    manager = new AgentManager();
    vi.mocked(disposeSessionQuietly).mockClear();
    ...
    // The manager must NOT call session.dispose() — that permanently poisons
    // the shared extension runtime (AgentSession.dispose() → runtime
    // .invalidate() latches staleMessage with ??=, never cleared). Instead it
    // routes teardown through disposeSessionQuietly, which aborts the agent
    // and runs cleanupSessionResources without invalidating the runtime.
    expect(sess.dispose).not.toHaveBeenCalled();
    expect(disposeSessionQuietly).toHaveBeenCalledOnce();
    expect(disposeSessionQuietly).toHaveBeenCalledWith(sess);
  });
