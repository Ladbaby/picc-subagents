...
describe("buildAgentPrompt", () => {
  ...
  describe("cache stability — env block at end", () => {
    /**
     * Anthropic's prompt cache hashes the marked block (the single system
     * prompt string here) to derive its cache key. To maximise cache hits
     * across calls within a session, the byte-stable prefix must be as long
     * as possible. The env block (cwd, branch, platform) is the only
     * per-call variable — placing it at the END ensures the cache key is
     * dominated by the stable identity + instructions.
     *
     * These tests guard against regressions of the env-block-at-end pattern.
     */
    const envA: EnvInfo = { isGitRepo: true, branch: "main", platform: "darwin" };
    const envB: EnvInfo = { isGitRepo: true, branch: "feature/cache-fix", platform: "linux" };
    ...
    const appendConfig: AgentConfig = {
      name: "cache-append",
      description: "Cache append agent",
      builtinToolNames: [],
      extensions: true,
      skills: true,
      systemPrompt: "Sub-agent specific instructions here.",
      promptMode: "append",
      inheritContext: false,
      runInBackground: false,
      isolated: false,
    ...
    function envBlockStart(prompt: string): number {
      return prompt.indexOf("# Environment");
    }

    it("same inputs produce byte-identical prompts (replace mode)", () => {
      const a = buildAgentPrompt(replaceConfig, "/repo", envA);
      const b = buildAgentPrompt(replaceConfig, "/repo", envA);
      expect(a).toBe(b);
    });

    it("same inputs produce byte-identical prompts (append mode)", () => {
      const a = buildAgentPrompt(appendConfig, "/repo", envA, "Parent system prompt.");
      const b = buildAgentPrompt(appendConfig, "/repo", envA, "Parent system prompt.");
      expect(a).toBe(b);
    });

    it("prefix is byte-stable across cwd changes (replace mode)", () => {
      const a = buildAgentPrompt(replaceConfig, "/repo/a", envA);
      const b = buildAgentPrompt(replaceConfig, "/repo/b", envB);
      const aPrefix = a.slice(0, envBlockStart(a));
      const bPrefix = b.slice(0, envBlockStart(b));
      expect(aPrefix).toBe(bPrefix);
    });

    it("prefix is byte-stable across env changes (replace mode)", () => {
      const a = buildAgentPrompt(replaceConfig, "/repo", envA);
      const b = buildAgentPrompt(replaceConfig, "/repo", envB);
      const aPrefix = a.slice(0, envBlockStart(a));
      const bPrefix = b.slice(0, envBlockStart(b));
      expect(aPrefix).toBe(bPrefix);
    });

    it("prefix is byte-stable across cwd changes (append mode)", () => {
      const parent = "Parent system prompt content.";
      const a = buildAgentPrompt(appendConfig, "/repo/a", envA, parent);
      const b = buildAgentPrompt(appendConfig, "/repo/b", envB, parent);
      const aPrefix = a.slice(0, envBlockStart(a));
      const bPrefix = b.slice(0, envBlockStart(b));
      expect(aPrefix).toBe(bPrefix);
    });

    it("prefix is byte-stable across env changes (append mode)", () => {
      const parent = "Parent system prompt content.";
      const a = buildAgentPrompt(appendConfig, "/repo", envA, parent);
      const b = buildAgentPrompt(appendConfig, "/repo", envB, parent);
      const aPrefix = a.slice(0, envBlockStart(a));
      const bPrefix = b.slice(0, envBlockStart(b));
      expect(aPrefix).toBe(bPrefix);
    });

    it("env block is the FINAL content block in replace mode", () => {
      const prompt = buildAgentPrompt(replaceConfig, "/repo", envA);
      expect(prompt.endsWith("Platform: darwin")).toBe(true);
      const afterEnv = prompt.slice(prompt.indexOf("Platform: darwin") + "Platform: darwin".length);
      expect(afterEnv).toBe("");
    });

    it("env block is the FINAL content block in append mode (no extras)", () => {
      const prompt = buildAgentPrompt(appendConfig, "/repo", envA, "Parent.");
      expect(prompt.endsWith("Platform: darwin")).toBe(true);
      const afterEnv = prompt.slice(prompt.indexOf("Platform: darwin") + "Platform: darwin".length);
      expect(afterEnv).toBe("");
    });

    it("extras appear AFTER the env block (replace mode)", () => {
      const extras = { memoryBlock: "# Memory\nSome persistent notes." };
      const prompt = buildAgentPrompt(replaceConfig, "/repo", envA, undefined, extras);
      const envIdx = prompt.indexOf("Platform: darwin");
      const memoryIdx = prompt.indexOf("# Memory");
      expect(envIdx).toBeGreaterThan(-1);
      expect(memoryIdx).toBeGreaterThan(envIdx);
    });

    it("extras appear AFTER the env block (append mode)", () => {
      const extras = {
        skillBlocks: [{ name: "test-skill", content: "Skill body." }],
      };
      const prompt = buildAgentPrompt(appendConfig, "/repo", envA, "Parent.", extras);
      const envIdx = prompt.indexOf("Platform: darwin");
      const skillIdx = prompt.indexOf("Preloaded Skill: test-skill");
      expect(envIdx).toBeGreaterThan(-1);
      expect(skillIdx).toBeGreaterThan(envIdx);
    });

    it("parent prompt remains the verbatim byte prefix in append mode", () => {
      // Claude Code relies on this: parent prompt forms an identical byte
      // prefix across all subagent invocations within a parent session.
      const parent = "PARENT_PROMPT_VERBATIM_TOKEN";
      const prompt = buildAgentPrompt(appendConfig, "/repo", envA, parent);
      expect(prompt.startsWith(parent)).toBe(true);
    });

    it("agent instructions come BEFORE the env block in append mode", () => {
      const prompt = buildAgentPrompt(appendConfig, "/repo", envA, "Parent.");
      const instructionsIdx = prompt.indexOf("Sub-agent specific instructions here.");
      const envIdx = prompt.indexOf("# Environment");
      expect(instructionsIdx).toBeGreaterThan(-1);
      expect(envIdx).toBeGreaterThan(instructionsIdx);
    });
  });
});
