# @local/pi-subagents (fork of @tintinweb/pi-subagents)

> **Local fork.** This is the same extension as
> [`@tintinweb/pi-subagents`](https://github.com/tintinweb/pi-subagents) with
> added support for `PI_SUBAGENT_MODEL` and `CLAUDE_CODE_SUBAGENT_MODEL` env
> vars — see the [Fork notes](#fork-notes-pi_subagent_model--claude_code_subagent_model)
> section below. To switch back to upstream, run
...
src/
  ...
  subagent-env.ts     # PI_SUBAGENT_MODEL / CLAUDE_CODE_SUBAGENT_MODEL env-var overrides (fork)
  ui/
...
This is a local fork of [`@tintinweb/pi-subagents`](https://github.com/tintinweb/pi-subagents) that
adds support for picking the **default subagent model from an environment variable**, mirroring
claude-code's [`CLAUDE_CODE_SUBAGENT_MODEL`](https://docs.claude.com/en/docs/claude-code/subagents)
hook (`utils/model/agent.ts` in
...
| _unset_ | — | Falls back to the existing precedence: caller `Agent({model: ...})` > frontmatter `model:` > inherit parent. |

### Examples

```bash
# Always run subagents on haiku (fuzzy alias resolves via the model registry)
export PI_SUBAGENT_MODEL=haiku

# Same idea, but using the claude-code-compatible env var
export CLAUDE_CODE_SUBAGENT_MODEL=anthropic/claude-haiku-4-5

# PI_SUBAGENT_MODEL wins when both are set
export PI_SUBAGENT_MODEL=haiku
export CLAUDE_CODE_SUBAGENT_MODEL=sonnet   # ignored
```

### Behavior

- The chosen model **overrides everything**: frontmatter `model:` pins and `model:` parameters
  passed to the `Agent` tool by the orchestrator. This matches claude-code's behavior where
  `CLAUDE_CODE_SUBAGENT_MODEL` is treated as the user's authoritative default.
- Resolution uses the same forgiving matcher as the `Agent` tool's own `model` parameter — fuzzy
  aliases (`haiku`, `sonnet`, `opus`), `provider/modelId`, optional date stamps, and provider
  fallback all work.
- A misconfigured env var (typo, unavailable model) surfaces a `Model not found: ...` tool error
  to the orchestrator. It does **not** silently fall back to the parent model — the env var is an
  explicit user choice and a silent fallback would mask typos.
- Whitespace-only values (`PI_SUBAGENT_MODEL=" "`) are treated as unset, so an empty shell
  variable doesn't accidentally pin every subagent to a bad model.
- Out-of-scope env-sourced models are warned about and proceed (same trust level as
  frontmatter-pinned agents). Scope enforcement is a runtime LLM-choice guard, not a
  user-config guard.
- Affects in-process `Agent` tool calls only. Cross-extension RPC spawns run in the caller
  process and pick up the same env vars automatically; if the caller is a different process,
  set the env var there.

### Verifying

```bash
# After exporting PI_SUBAGENT_MODEL=haiku
pi -e ./extensions/pi-subagents

# Spawn an Explore agent — widget should show "haiku" as the modelName tag
# even though Explore's default frontmatter pins anthropic/claude-haiku-4-5
# exactly and the parent session is on opus.
```

To uninstall and revert to upstream, remove the `extensions/pi-subagents/` directory and run
`pi install npm:@tintinweb/pi-subagents`.
