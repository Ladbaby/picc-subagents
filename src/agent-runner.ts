...
/**
 * Tear down a subagent session WITHOUT invalidating the shared extension
 * runtime.
 *
 * AgentSession.dispose() is deliberately NOT used: it calls
 * `this._extensionRunner.invalidate(...)`, which sets the extension runtime's
 * `staleMessage` with `??=` — a one-way latch that is never cleared. Every
 * extension api bound to that runtime then throws "This extension ctx is
 * stale" on sendMessage for the rest of the process. Subagent sessions run
 * in-process (their extension factories execute against the same module
 * singleton state as the parent session's), so the poison would permanently
 * break cron schedulers (pi-loop) and any other extension that captured a `pi`
 * reference — even after the subagent has finished.
 *
 * Safe teardown instead:
 *   - `session.agent.abort()` — stop any in-flight agent work (a safe no-op
 *     after the run has settled; `forwardAbortSignal` already aborts on the
 *     caller's signal).
 *   - `cleanupSessionResources(sessionId)` — run per-session resource
 *     cleanups (bash watchers, etc.) exactly as dispose() would.
 *   - drop the reference — the session and its Agent are per-subagent and
 *     become garbage once the manager removes the record. Event
 *     subscriptions (turn tracking, text collection, abort forwarding) are
 *     unsubscribed in runAgent's finally block, so no listeners leak.
 */
export function disposeSessionQuietly(session: AgentSession): void {
  try {
    session.agent.abort();
  } catch {
    /* best-effort */
