...
import { disposeSessionQuietly, resumeAgent, runAgent, type ToolActivity } from "./agent-runner.js";
...
export class AgentManager {
  ...
  /**
   * Send a steering message to an agent from the UI. A live session delivers it
   * now — it interrupts the agent after its current tool execution and appears
   * as a user message. If the session isn't ready yet, the message is queued on
   * `pendingSteers` and flushed when the session is created. Returns false if
   * the agent can't accept steering (unknown id, or no longer running/queued).
   */
  ...
  private removeRecord(id: string, record: AgentRecord): void {
    if (record.session) {
      // NOT session.dispose(): it permanently poisons the shared extension
      // runtime (staleMessage ??= latch) for the whole process. See
      // disposeSessionQuietly in agent-runner.ts.
      disposeSessionQuietly(record.session);
  ...
  dispose() {
    ...
    this.queue = [];
    for (const record of this.agents.values()) {
      // NOT session.dispose() — see disposeSessionQuietly in agent-runner.ts.
      if (record.session) disposeSessionQuietly(record.session);
    }
    this.agents.clear();
