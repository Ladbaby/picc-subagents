...
import { pickSubagentEnvModel } from "./subagent-env.js";
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  pi.registerTool(defineTool({
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      // anthropic/claude-code's `utils/model/agent.ts`.
      let modelFromEnv = false;
      const envSubagentModel = pickSubagentEnvModel();
      if (envSubagentModel !== undefined) {
        const envResolved = resolveModel(envSubagentModel, ctx.modelRegistry);
        if (typeof envResolved === "string") {
          // Resolution failed — surface the error to the orchestrator instead
          // of silently falling back to the parent model, since the user set
          // the env var explicitly (matches claude-code fail-fast behavior).
          return textResult(envResolved);
        }
        model = envResolved;
        modelFromEnv = true;
      } else if (resolvedConfig.modelInput) {
      ...
      if (isScopeModelsEnabled() && model) {
        ...
        if (allowed && !isModelInScope(model, allowed)) {
          // Env-sourced / frontmatter-pinned / parent-inherited: trust the user.
          // Only caller-supplied `Agent({model: ...})` out-of-scope → hard error.
          if (resolvedConfig.modelFromParams && !modelFromEnv) {
